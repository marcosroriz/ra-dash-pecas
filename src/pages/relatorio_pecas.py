...
@callback(
    [Output("tabela-relatorio-pecas-gerais", "rowData"), Output("boxplot-vida-util-total-pecas", "figure")],
    [
...
def tabela_relatio_peças(datas, lista_modelos, peça):
    ...
    fig.update_layout(xaxis_title="Peça", yaxis_title="Duração (km)", boxmode="group", template="plotly_white")

    return df.to_dict("records"), fig


@callback(
    [Output("grafico-barras-qtd-peças-mes", "figure"), Output("grafico-barras-valor-peças-mes", "figure")],
    [
        Input("input-intervalo-datas-pecas-os", "value"),
        Input("input-select-modelo-veiculos-relatorio-pecas", "value"),
        Input("input-select-peca-relatorio", "value"),
...
def grafico_barras_qtd_valor_peças_mes(datas, lista_modelos, peça):
    ...
    fig_qtd.update_traces(hovertemplate="<b>Mês:</b> %{x}<br><b>Qtd. peças:</b> %{y}<extra></extra>")
    ...
    fig_valor.update_traces(
        marker_color="red", hovertemplate="<b>Mês:</b> %{x}<br><b>Valor esperado:</b> R$ %{y:,.2f}<extra></extra>"
    )
...
def corrige_input_pecas(datas, lista_modelos, lista_pecas):
    ...
    lista_options = [{"label": f"{row['nome_peça']}", "value": row["nome_peça"]} for _, row in df_pecas.iterrows()]
                        ...
                        [
                            ...
                            dbc.Row(
                                [
                                                ...
                                                dbc.Row(
                                                    [
                                                        ...
                                                        dbc.Row(
                                                            [
                                                                dbc.Col(
                                                                    gera_labels_inputs("tabela-relatorio-pecas"),
                                                                    width=True,
                                                                ),
                                                                dbc.Col(
                                                                    html.Div(
                                                                        [
                                                                            ...
                                                                            dcc.Download(
                                                                                id="download-excel-tabela-relatorio-pecas"
                                                                            ),
                                                                        ],
                                    ...
                                    dmc.Space(h=20),
                                    html.H6(
                                        "Legenda: Classificação (Vida útil consumida)", style={"marginBottom": "5px"}
                                    ),
                                    html.Div(
                                        [
                                            html.Div(
                                                [
                                                    ...
                                                    html.Span(
                                                        "A — Dentro da estimativa - Menor que 65%",
                                                        style={"fontSize": "18px"},
                                                    ),
                                                ],
